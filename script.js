document.addEventListener('DOMContentLoaded',function(){initializeWebsite()});function initializeWebsite(){initNavigation();initScrollEffects();initGallery();initForms();initVideoPlayer();initScrollAnimations();initThemeToggle();initSoundToggle();initBackToTop();initLoadingOverlay();setTimeout(()=>{document.body.classList.add('loaded');triggerInitialAnimations()},100)}
...
function initQuoteForm(){const quoteForm=document.getElementById('quoteForm');if(!quoteForm)return;quoteForm.addEventListener('submit',async(e)=>{e.preventDefault();const formData=new FormData(quoteForm);const formObject=Object.fromEntries(formData);const submitButton=quoteForm.querySelector('button[type="submit"]');const originalText=submitButton.textContent;submitButton.textContent='Sending...';submitButton.disabled=!0;try{const emailContent=`
New Quote Request from ${formObject.name}
...
Please respond promptly to this potential customer.
            `.trim();const subject=encodeURIComponent('New Quote Request - Groves Real Estate Painting');const body=encodeURIComponent(emailContent);const mailtoLink=`mailto:Grovesrealestate@gmail.com?subject=${subject}&body=${body}`;window.location.href=mailtoLink;showNotification('Quote request prepared! Your email client should open shortly.','success');quoteForm.reset()}catch(error){console.error('Error preparing quote request:',error);showNotification('There was an error preparing your quote request. Please try again.','error')}finally{submitButton.textContent=originalText;submitButton.disabled=!1}})}
function initReviewForm(){const reviewForm=document.querySelector('.review-form');if(!reviewForm)return;reviewForm.addEventListener('submit',async(e)=>{e.preventDefault();const formData=new FormData(reviewForm);const formInputs=reviewForm.querySelectorAll('input, select, textarea');const name=formInputs[0].value;const rating=formInputs[1].value;const review=formInputs[2].value;const submitButton=reviewForm.querySelector('button[type="submit"]');const originalText=submitButton.textContent;submitButton.textContent='Submitting...';submitButton.disabled=!0;try{const emailContent=`
New Customer Review
...
This review was submitted through the website contact form.
            `.trim();const subject=encodeURIComponent('New Customer Review - Groves Real Estate Painting');const body=encodeURIComponent(emailContent);const mailtoLink=`mailto:Grovesrealestate@gmail.com?subject=${subject}&body=${body}`;window.location.href=mailtoLink;showNotification('Thank you for your review! We appreciate your feedback.','success');reviewForm.reset()}catch(error){console.error('Error submitting review:',error);showNotification('There was an error submitting your review. Please try again.','error')}finally{submitButton.textContent=originalText;submitButton.disabled=!1}})}
...
window.copyEmailContent=copyEmailContent;window.closeEmailDialog=closeEmailDialog;window.addEventListener('error',(e)=>console.error('JS Error:',e.error))